...
def scanDirectory
    (fileSystem : FileSystem)
    (config : AppConfig)
    (directory : FilePath) : Z Unit FileError ScanResult :=
  scanDirectoryWithWorkers fileSystem config directory
    defaultScannerWorkerCount defaultScannerQueueCapacity

...
To write a Graphviz DOT execution diagram, import `Zenith.Debug`:

```lean
import Zenith.Debug

def run : IO (Exit Empty Unit) :=
  Zenith.Debug.runWithGraphviz (Z.succeed ()) "diagrams/example.dot" "example"
```

`Zenith.Debug.runWithGraphviz` opens the DOT file, installs the Graphviz observer,
